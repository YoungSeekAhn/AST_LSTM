...
class MultiBranchLSTM(nn.Module):
    def __init__(
        ...
        enc_out = self.enc_p.out_dim + self.enc_f.out_dim + self.enc_d.out_dim 
    ...
    def forward(self, xp, xf, xd):
        ...
        h_list = [t for t in (hp, hf, hd) if t is not None]
...
def training_LSTM(payload, cfg, LOAD_DATASET_FILE = True, TRAIN_PLOT = True):
    ...
    # 언팩 오타 수정(검증 세트)
    Xdate_tr, Xp_tr, Xf_tr, Xd_tr, Y_tr = \
        payload["Xdate_tr"], payload["Xp_tr"], payload["Xf_tr"], payload["Xd_tr"], payload["Y_tr"]

    Xdate_va, Xp_va, Xf_va, Xd_va, Y_va = \
        payload["Xdate_va"], payload["Xp_va"], payload["Xf_va"], payload["Xd_va"],  payload["Y_va"]

    Xdate_te, Xp_te, Xf_te, Xd_te, Y_te = \
        payload["Xdate_te"], payload["Xp_te"], payload["Xf_te"], payload["Xd_te"],  payload["Y_te"]
